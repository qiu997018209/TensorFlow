...
tf.flags.DEFINE_float("dropout_keep_prob", 1
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN(
            sequence_length=my_data.max_document_length,
        ...
        out_dir = os.path.abspath(os.path.join(my_data.file_path, "runs"))
        ...
        # Generate batches
        batches= my_data.batch_iter(FLAGS.batch_size, FLAGS.num_epochs,rate=0)
        for batch in batches:
            ...
            if(1 == current_step):
                print("batch_x:%s,batch_y:%s,dev_x:%s,dev_y:%s"%(batch_x.shape,batch_y.shape,dev_x.shape,dev_y.shape))
