...
tf.app.flags.DEFINE_integer('max_decode_steps', 100,
...
class DecodeIO(object):
  ...
  def ResetFiles(self):
    ...
    self._ref_file = open(
        os.path.join(self._outdir, 'ref%d'%timestamp), 'w',encoding='utf-8')
    self._decode_file = open(
        os.path.join(self._outdir, 'decode%d'%timestamp), 'w',encoding='utf-8')
...
class BSDecoder(object):
  ...
  def DecodeLoop(self):
    ...
    while step < FLAGS.max_decode_steps:
      time.sleep(DECODE_LOOP_DELAY_SECS)#100
  ...
  def _Decode(self, saver, sess):
    ...
    for _ in xrange(FLAGS.decode_batches_per_ckpt):
        #每次将一条quest复制batch_size条，输入到BeamSearch里，取出最优结果,重复batch_size次
  ...
  def _DecodeBatch(self, article, abstract, output_ids):
    ...
    print('abstract=%s,decoded=%s'%(abstract,decoded_output))
